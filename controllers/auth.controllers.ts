...
// Internal services
import { generateAccessAndRefreshTokens, validateRefreshToken, isPasswordCorrect } from '@/services/adminService';
...
export async function logoutUser(request: NextRequest) {
  try {
    ...
    // ✅ Build JSON response
    const response: NextResponse = NextResponse.json(new ApiResponse(200, null, 'Logout successful'), {
      status: 200
    });
...
export async function refreshToken(request: NextRequest) {
  try {
    ...
    if (role === 'admin') {
      ...
      const { accessToken, refreshToken: newRefreshToken } = await generateAccessAndRefreshTokens('admin', adminId);
    ...
    if (role === 'student') {
      ...
      // ♻️ Generate new tokens
      const { accessToken, refreshToken: newRefreshToken } = await generateAccessAndRefreshTokens('student', studentId);
...
export async function studentLogin(request: NextRequest) {
  try {
    ...
    // ✅ Build JSON response
    const response: NextResponse = NextResponse.json(new ApiResponse(200, null, 'Student login successful'), {
      status: 200
    });
