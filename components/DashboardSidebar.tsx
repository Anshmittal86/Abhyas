...
export default function DashboardSidebar({ sidebarOpen, role }: DashboardSidebarProps) {
	...
	const navItemClass = (active: boolean) =>
		`flex items-center px-3 py-4 rounded border transition ${
			active ? 'border-amber-600 text-amber-600 bg-surface' : 'border-transparent hover:text-accent'
		}`;
	...
	return (
		<aside
			...
			} sm:translate-x-0 border-r border-default`}
		>
			<div className="h-full px-4 py-6 overflow-y-auto bg-sidebar will-change-transform">
				<h1 className="text-lg font-bold text-primary mb-8">
				...
				<ul className="space-y-3 text-sm font-medium">
					...
					<li>
						<Link
							...
							className={navItemClass(
								isActive(isAdmin ? '/admin/dashboard' : '/student/dashboard')
							)}
						>
							Dashboard
						</Link>
						...
						<>
														...
														<ul className="ml-3 mt-1 space-y-1">
															...
															: courseChapters[course.id]?.length ?
																courseChapters[course.id].map((ch) => (
																	<li
																		key={ch.id}
																		className="text-xs text-muted text-shadow-amber-200"
																	>
																		{ch.title}
																	</li>
																))
							...
							<li>
								<Link
									...
									className={navItemClass(isActive('/student/results'))}
								>
									Test Results
								</Link>
							</li>
							...
							<li>
								<Link
									...
									className={navItemClass(isActive('/student/profile'))}
								>
									My Profile
								</Link>
							</li>
					...
					<li className="pt-4 mt-4 border-t border-default">
						<Button
							variant="outline"
							className={navItemClass(isActive(isAdmin ? '/admin/logout' : '/logout'))}
