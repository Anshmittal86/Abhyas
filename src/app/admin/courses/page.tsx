    ...
    <main className="flex-1 overflow-auto px-8 py-6 space-y-6">
      ...
      <div className="bg-surface border border-default rounded-xl overflow-hidden">
        ...
        {courses.length === 0 ? (
          ...
          courses.map((c) => (
            <div key={c.id} className="grid grid-cols-12 gap-3 px-4 py-4 border-b border-default last:border-b-0 items-center">
              <div className="col-span-4">
                <div className="text-primary font-medium">{c.title}</div>
                {c.description ? <div className="text-xs text-muted line-clamp-1">{c.description}</div> : null}
              </div>
              ...
              <div className="col-span-2">
                <span
                  className={[
                    ...
                    c.isActive ? 'border-accent-success text-accent-success' : 'border-accent-error text-accent-error'
                  ].join(' ')}
                >
              ...
              <div className="col-span-2 flex gap-2">
                <Button type="button" variant="outline" size="sm" className="cursor-pointer" onClick={() => openView(c)}>
                  View
                </Button>
                <Button type="button" variant="outline" size="sm" className="cursor-pointer" onClick={() => openEdit(c)}>
                  Edit
                </Button>
              </div>
            </div>
      ...
      <CourseFormSheet open={createOpen} onOpenChange={setCreateOpen} mode="create" onSuccess={fetchCourses} />

      <CourseFormSheet open={editOpen} onOpenChange={setEditOpen} mode="edit" course={courseToEdit} onSuccess={fetchCourses} />
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <span
                    className={[
                      'inline-flex items-center rounded-full px-2 py-1 text-xs border',
                      selectedCourse.isActive ? 'border-accent-success text-accent-success' : 'border-accent-error text-accent-error'
                    ].join(' ')}
                  >
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Button type="button" size="sm" className="cursor-pointer" onClick={openAddChapter}>
                      Add Chapter
                    </Button>
                  </div>
                  {selectedCourse.chapters?.length ? (
                    <div className="rounded-lg border border-default divide-y divide-default">
                      {[...selectedCourse.chapters]
                        ...
                        .map((ch) => (
                          <div key={ch.id} className="flex items-center justify-between gap-2 px-3 py-2 text-primary text-sm">
                            <div>
                            ...
                            <div className="flex gap-1 shrink-0">
                              <Button type="button" variant="ghost" size="sm" className="cursor-pointer h-8" onClick={() => openEditChapter(ch)}>
                                Edit
                              </Button>
                              <Button
                                type="button"
                                ...
                                onClick={() => selectedCourse && deleteChapter(selectedCourse.id, ch.id)}
                    ...
                    <div className="text-secondary text-sm py-2">No chapters yet. Click &quot;Add Chapter&quot; to add one.</div>
                  )}
                </div>
                <div className="pt-4 flex gap-2">
                  <Button type="button" variant="outline" className="cursor-pointer" onClick={() => openEdit(selectedCourse)}>
                    Edit Course
                  </Button>
                  <SheetClose asChild>
      ...
      {selectedCourse && (
        <>
          <ChapterFormSheet
            ...
            defaultOrderNo={selectedCourse.chapters?.length ? Math.max(...selectedCourse.chapters.map((c) => c.orderNo)) + 1 : 1}
